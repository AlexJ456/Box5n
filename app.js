...
index 391d1a9cd1f22b5fd3491ead78da39fc5f2a3514..020d3ec5f0455b2d3d5ad1a8ecf41a5bca8deb5f 100644
--- a/app.js
+++ b/app.js
@@ -18,50 +18,71 @@ document.addEventListener('DOMContentLoaded', () => {
         pulseStartTime: null
     };
...
@@ -89,69 +110,67 @@ document.addEventListener('DOMContentLoaded', () => {
...
@@ -172,105 +191,184 @@ document.addEventListener('DOMContentLoaded', () => {
...
+        const topMargin = 20;
+        const sizeWithoutBreath = Math.min(baseSize, canvas.height - topMargin * 2);
+        const verticalOffset = Math.min(canvas.height * 0.18, 110);
+        const preferredTop = canvas.height / 2 + verticalOffset - sizeWithoutBreath / 2;
+        const top = Math.max(topMargin, Math.min(preferredTop, canvas.height - sizeWithoutBreath - topMargin));
+        const left = (canvas.width - sizeWithoutBreath) / 2;
+
+        const now = timestamp;
...
+        ctx.strokeStyle = hexToRgba(accentColor, showTrail ? 0.8 : 0.45);
+        ctx.shadowColor = hexToRgba(accentColor, 0.5);
+        ctx.shadowBlur = showTrail ? 15 : 8;
+        ctx.beginPath();
...
+                html += `
+                    <div class="phase-item ${index === state.count ? 'active' : ''}">
...
@@ -320,29 +418,33 @@ document.addEventListener('DOMContentLoaded', () => {
                     </button>
                 </div>
             `;
         }
